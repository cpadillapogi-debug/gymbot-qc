...
export function matchOwnerFaq(userMessage, ownerFaqs){
  ...
  for(const faq of ownerFaqs){
    ...
    // Require at least half the owner's question words to appear in the
    // customer's message, and keep only the strongest match across all
    // of the owner's saved FAQs (in case more than one loosely overlaps).
