...
import { generateId, isValidEmail, sanitizeRecords } from "../utils.js";
...
export function login({ email, password, rememberMe }){
  ...
  if(!user || !verifyPassword(password, user.passwordHash)){
    logSystemEvent({ level: SYSTEM_LOG_LEVELS.WARNING, category: SYSTEM_LOG_CATEGORIES.LOGIN_ATTEMPT, message: `Failed login attempt for ${cleanEmail}` });
  ...
  logSystemEvent({ level: SYSTEM_LOG_LEVELS.INFO, category: SYSTEM_LOG_CATEGORIES.LOGIN_ATTEMPT, message: `Successful login: ${cleanEmail}` });
  return { ok:true, user: toSafeUser(user) };
}
