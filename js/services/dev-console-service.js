...
/**
 * Merges `patch` into an already-recorded log entry's `meta` object.
 * Used to attach IP-geolocation to a login entry once the (async, best-
 * effort) lookup resolves — the entry is written synchronously at login
 * time with device info only, then patched in place a moment later so
 * the UI doesn't have to block the login flow on a network call.
 * Best-effort only, same as logSystemEvent — never throws.
 * @param {string} id log entry id, as returned by logSystemEvent()
 * @param {object} patch fields to merge into entry.meta
 */
export function updateSystemLogMeta(id, patch){
  try{
    const all = getAllSystemLogs();
    const idx = all.findIndex(l => l.id === id);
    if(idx === -1) return null;
    all[idx] = Object.assign({}, all[idx], { meta: Object.assign({}, all[idx].meta || {}, patch || {}) });
    storage.setJSON("systemLogs", all);
    return all[idx];
  }catch(err){
    return null;
  }
