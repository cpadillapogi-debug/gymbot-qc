...
import { matchFaqIntent, matchOwnerFaq } from "./faq-response-service.js";
 ...
 */
export async function callGemini(userMessage, history, memorySummary, gymInfoOverride, ownerFaqs){
  // PATCH: answer from the gym owner's own FAQs first, then the fixed
  // generic FAQ list — no API call, no key needed, no "model retired"
  // surprises. Only falls through to Gemini below if the message doesn't
  // match anything in either list.
