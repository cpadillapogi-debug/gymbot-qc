...
export async function callGemini(userMessage, history, memorySummary, gymInfoOverride){
  // Phase 15: in proxy mode, the real key lives server-side only — the
  // browser never holds one, so the old "must have a locally-saved key"
  // gate would incorrectly block every call. Only require a local key
  // when calling Google directly (no proxy configured).
  const apiKey = loadApiKey().trim();
  if(!CONFIG.AI_PROXY_URL && !apiKey){
...
export async function testGeminiConnection(apiKeyOverride){
  ...
  // Phase 15: same proxy-mode exception as callGemini() above.
  if(!CONFIG.AI_PROXY_URL && !apiKey) return { ok:false, reason:"no_key" };
  if(!isOnline()) return { ok:false, reason:"offline" };
