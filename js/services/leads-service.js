...
/** Replaces this gym's leads entirely with `leads` (every record is
 *  force-tagged with this gymId, ignoring whatever gymId it arrived
 *  with — used by owner-backup-service.js's restore, so a backup file
 *  can never overwrite another gym's leads no matter what it contains).
 *  Other gyms' leads are left untouched. */
export function replaceLeadsForGym(gymId, leads){
  if(!gymId) return false;
  const others = getAllLeadsRaw().filter(l => !(l && l.gymId === gymId));
  const incoming = sanitizeRecords(Array.isArray(leads) ? leads : [], ["id"])
    .map(l => Object.assign({}, l, { gymId }));
  return saveAllLeadsRaw(others.concat(incoming));
}
