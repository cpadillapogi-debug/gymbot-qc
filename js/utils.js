...
/** Best-effort, dependency-free User-Agent parse for the Security Center's
 *  "device" column. Deliberately simple pattern matching — good enough to
 *  show "Chrome on Windows" / "Safari on iPhone", not a full UA-parsing
 *  library. Never throws; unknown/empty input falls back to "Unknown". */
export function parseDeviceInfo(userAgent){
  const ua = typeof userAgent === "string" ? userAgent : "";
  if(!ua) return { browser: "Unknown", os: "Unknown", deviceType: "Unknown", label: "Unknown device" };
  ...
  const deviceType = /iPhone|Android.*Mobile/.test(ua) ? "Mobile"
