...
export const CONFIG = Object.freeze({
  STORAGE_KEYS: Object.freeze({
    ...
    // Phase 16
    loginLockouts: "gymbot_login_lockouts"            // { [email]: { attempts:[isoString,...], lockedUntil: isoString|null } } — see auth-service.js
  }),
  ...
  // Phase 16: login lockout (brute-force throttling). Tracked per email,
  // in localStorage (see auth-service.js's checkLoginLockout()) — this is
  // a client-only app with no server to enforce a "real" rate limit
  // against, so this raises the cost of guessing from the same browser/
  // device rather than being a hard security boundary. A determined
  // attacker with multiple browsers/devices isn't stopped by this alone.
  LOGIN_LOCKOUT_MAX_ATTEMPTS: 5,        // failed attempts before lockout
  LOGIN_LOCKOUT_WINDOW_MS: 15 * 60 * 1000,   // attempts older than this don't count
  LOGIN_LOCKOUT_DURATION_MS: 15 * 60 * 1000, // how long a lockout lasts once triggered,
  ...
  // Phase 15: AI proxy (see /server/gemini-proxy-worker.js and docs/AI_PROXY_SETUP.md).
  // When set to a deployed proxy's URL, gemini-service.js calls THIS instead of
  // Google directly — the real Gemini key then lives only as a server-side
  // secret (e.g. a Cloudflare Worker env var), never in the browser or in
  // localStorage. Leave "" to keep the old direct-call behavior (client-side
