...
export function showBookingForm(){
  ...
  card.querySelector("#confirmBookingBtn").addEventListener("click", () => confirmBooking(card));
}
...
function confirmBooking(card){
  ...
  // This public demo widget has no logged-in gym — see DEMO_GYM_ID's
  // definition in config.js for why these leads are tagged separately
  // from any real gym's CRM data.
  ...
  // Keep AppState in sync so the dashboard re-renders.
