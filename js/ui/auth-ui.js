...
if (hint) {
  ...
  form.addEventListener("submit", e => {
    ...
    setTimeout(() => {
      ...
      // Phase 16: hand off a one-time "new device" flag for the
      // destination dashboard to toast on first render. sessionStorage
      // (not the shared `storage` adapter) so it's naturally scoped to
      // this tab and never lingers across a later normal login.
      if(result.newDeviceDetected){
        try{ window.sessionStorage.setItem("gymbot_new_device_alert", "1"); }catch(err){ /* best-effort */ }
      }
