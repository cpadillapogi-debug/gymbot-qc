  ...
  return `
    <div class="owner-panel">
      ...
      <p class="help-text" style="margin-top:0;">${access.aiEnabled
        ? "Share this on Messenger, put it as a button on your Facebook Page, or send it directly to customers."
        : "Your AI Receptionist is currently disabled (see the status above), so this link currently shows a temporarily-unavailable notice instead of the chat."}</p>

      <div class="owner-file-row">
        <a href="${escapeHtml(widgetUrl)}" target="_blank" rel="noopener noreferrer" class="btn btn-primary btn-sm">Open your chat</a>
        <button class="btn btn-ghost btn-sm" type="button" id="ownerEmbedLinkCopyBtn" data-copy-target="ownerEmbedLinkHidden">Copy link</button>
      </div>
      <input type="text" id="ownerEmbedLinkHidden" readonly value="${escapeHtml(widgetUrl)}" style="position:absolute;left:-9999px;" aria-hidden="true" tabindex="-1">

      <details style="margin-top:16px;">
        <summary class="help-text" style="cursor:pointer;">Advanced: embed this on your own website</summary>
        <div class="owner-field" style="margin-top:10px;">
          <label for="ownerEmbedIframe">Paste this into your website's HTML</label>
